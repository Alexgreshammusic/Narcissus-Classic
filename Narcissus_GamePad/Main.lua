...
local function SelectActionGroup(name, mode)
    if ACTION_GOUPS[name] then
        ...
        for k, actionGroup in pairs(ACTION_GOUPS) do
            if name ~= k then
                actionGroup:OnDeactive();
            else
...
function Loader:Init()
    ...
    if NarciEquipmentSetManager then
        hooksecurefunc(NarciEquipmentSetManager, "ShowUI", function()
            SelectActionGroup("ClassicEquipmentSet");
        end);
    end
end
